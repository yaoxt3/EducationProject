...
def impedance_control_with_target(target):
...
def mujocoJointFunc():
    ...
    while True:
        ...
        if su_flag and flag <= 0:
            env.sim.data.xfrc_applied[env.sim.model.body_name2id("object"), :] = np.array([0, 30, 0, 0, 0, 0])
            for i in range(70):
                env.sim.step()
                env.render()
                sleep(0.05)
...
def mujocoWrenchFunc():
    ...
    while True:
        # env.sim.data.xfrc_applied[env.sim.model.body_name2id("panda0_link7"), :] = np.array([0, 0, 0, 0, 0, 0])
        jtor, su_flag = joint_controller.impedance_controller_joint()
        # jtor_wrench = joint_controller.impedance_controller_wrench(jtor)
        robot.set_joint_torque(jtor)
        env.sim.step()
        env.render()
