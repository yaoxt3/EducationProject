...
_SITE_NAME = 'ee_joint'
_JOINTS = ['panda0_joint1', 'panda0_joint2', 'panda0_joint3', 'panda0_joint4', 'panda0_joint5', 'panda0_joint6',
           'panda0_joint7']
_TOL = 1.2e-8
_MAX_STEPS = 150
_MAX_RESETS = 10
...
def impedance_control_integration(ctrl_rate):
    ...
    global ik_target_ee_pos, ik_target_ee_quat, ik_current_jpos, ik_result, ik_solver, Ja_privious_step, K_maxmax
    count = 0
    threshold = 0.0000005
        ...
        while error > threshold:
            ...
            while force_norm > 0:
                if count == 0:
                    ...
                    target_joint_vel = env.joint_velocities()[:7]
                    curr_vel_ee_ee, curr_omg_ee_ee = env.get_ee_velocity()
                    initial_yvel_control = curr_vel_ee_ee[1] # the speed of ee at the moment of contact
                    step_contact = step
                    print("step_contact", step_contact)
                    delta_target = 0.38 - curr_ee[1]
                    K_max = force_norm/delta_target#+10*step
            ...
            # if object's velocity < 0.01, stop the robot
            ...
            if count > 0:  # controller 1, in-contact phase
                ...
                desired_torque_control1 = (
                            np.multiply(np.array(position_error_control1), np.array(300)) + np.multiply(vel_pos_error,
                                                                                                      kd))
                ###pre_design parameter for optimization
                Ja = env.get_ee_jacobian("ee_joint")
                Ja_y = env.get_ee_jacobian("ee_joint")[1, :]
                y_position_error = Ja_y.dot(position_error) #useful
                y_position_error_A =  np.multiply(vel_pos_error, kd) + desired_torque_control1
                y_vel_error = Ja_y.dot(y_position_error_A) # useful
                delta_Ja = Ja - Ja_privious_step
                dot_Ja = delta_Ja / 0.002
                Ja_privious = Ja.copy()
                Ja_privious_step = Ja_privious
                delta_Ja_qvel = delta_Ja[1, :].dot(env.joint_position()[:7]) #useful
             #   print("111111", y_position_error+y_vel_error+delta_Ja_qvel)
                ee_vel_y = Ja_y.dot(env.joint_velocities()[:7]) #the speed of y
                t_time = 0.002
             #   print("22222", ee_vel_y)
                param = 0.5*y_position_error*t_time*t_time-0.000001
                print("param", param)
                if K_maxmax < 1100:
                    opti = casadi.Opti()
                    u = opti.variable(1)
                    opti.minimize((u) ** 2)
                    opti.subject_to((curr_vel[1] * t_time + 0.5 * (
                                delta_Ja_qvel + y_vel_error) * t_time * t_time) + param * u <= delta_target / (83))
                    opti.subject_to(
                        0.012 * initial_yvel_control <= ((abs(delta_Ja_qvel) + abs(y_vel_error)) * t_time) + abs(
                            y_position_error * t_time + 0.00000000001) * u)
                    # optimal control
                    # u = ca.SX.sym("u")
                    # opti = casadi.Opti()
                    # u = opti.variable(1)
                    # opti.minimize((u)**2)
                    # opti.subject_to(aaaaa*u >=-delta_target/1000+env.joint_velocities()[:7])#, np.array(u)) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos)))*((step-step_contact)*0.002)*((step-step_contact)*0.002))
                    # # opti.subject_to(np.linalg.pinv(MassMatrix).dot(((np.multiply(np.array(position_error), np.array(u)) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos))*((step-step_contact)*0.002) >=  0.001*env.joint_velocities()[:7])
                    opti.subject_to(u >= 0)
                    opti.solver('ipopt')
                    sol = opti.solve()
                    K_maxmax = sol.value(u)
                    print("K_max", K_maxmax)
                    # print(-delta_target/1000+env.joint_velocities()[:7])
                    print(sol.value(u))
                else:
                    K_maxmax = 1260
                ...
                # acc = np.linalg.pinv(MassMatrix).dot(((np.multiply(np.array(position_error), np.array(u)) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos))
                # a1 = env.joint_velocities()[:7]+1/2*acc*((step-step_contact)*0.002)*((step-step_contact)*0.002)
                # b1 = delta_target/1000
                # a2 = 0.001*env.joint_velocities()[:7]
                # b2 = acc*((step-step_contact)*0.002)
                # constraints = [env.joint_velocities()[:7]+1/2*np.linalg.pinv(MassMatrix) @ np.array(position_error) @ np.array(u) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos)) ((step-step_contact)*0.002)*((step-step_contact)*0.002) <= delta_target/1000,
                #                0.001*env.joint_velocities()[:7] <= np.linalg.pinv(MassMatrix).dot(((np.multiply(np.array(position_error), np.array(u)) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos))*((step-step_contact)*0.002),
                #                np.linalg.pinv(MassMatrix).dot(((np.multiply(np.array(position_error), np.array(u)) + np.multiply(vel_pos_error, kd)) + desired_torque_control1)-joint_controller.dynamics.gravityforces(robot.joint_pos)) <= 0,
                #                0 <= u]
                # problem = cp.Problem(objective, constraints)
                # problem.solve()
                # u = u.value[:1 + 1]
                # print("u", u)
                desired_torque = (np.multiply(np.array(position_error), np.array(K_maxmax)) + np.multiply(vel_pos_error, 1)) + desired_torque_control1
...
if __name__ == "__main__":
    ...
    while True:
        if su_flag:
            ...
            #     desired_torque_initial = (np.multiply(np.array(position_error_initial), np.array(1800)) + np.multiply(
            #         vel_pos_error_initial, 400))
            #     MassMatrix_intial = joint_controller.dynamics.MassMatrix()
            #     desired_torque_initial = MassMatrix_intial.dot(
            #         desired_torque_initial) + joint_controller.dynamics.gravityforces(robot.joint_pos)
    ...
    K_maxmax = 0
    ori_Ja = env.get_ee_jacobian("ee_joint")
    Ja_privious_step = ori_Ja
