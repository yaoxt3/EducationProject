...
def inverse_kinematics_thread():
    global ik_physics2, ik_target_pos, ik_target_quat, ik_current_pos, ik_joint_pos, ik_solver
    
    while True:
        print(f"ik solver thread")
        if ik_solver:
            print(f"starting ik solver")                
            count = 0
            ik_physics2.named.data.qpos[_JOINTS] = ik_current_pos
            while True:
                result = ik.qpos_from_site_pose(
                    physics=ik_physics2,
                    site_name=_SITE_NAME,
...
def impedance_control_integration(ctrl_rate):
    ...
    global ik_target_pos, ik_target_quat, ik_current_pos, ik_joint_pos, ik_solver
    count = 0
    threshold = 0.0000005
